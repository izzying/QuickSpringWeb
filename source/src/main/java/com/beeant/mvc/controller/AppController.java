...
public class AppController {
    ...
    public ModelAndView login(UserNamePassword user, HttpServletRequest request,HttpServletResponse response) {
        ModelAndView mav = new ModelAndView(shiroFilter.getLoginUrl());
        mav.addObject("user",user);
        Subject subject = SecurityUtils.getSubject();
        if (subject.isAuthenticated()) {
            return new ModelAndView("redirect:/".concat(shiroFilter.getSuccessUrl()));
        }
        ...
        if(null != action && "login".equals(action)){
            if (StringUtils.isEmpty(user.getUsername()) || StringUtils.isEmpty(user.getPassword())) {
                if (StringUtils.isEmpty(user.getPassword())) {
                    mav.addObject("password", "密码不能为空");
                }
                if (StringUtils.isEmpty(user.getUsername())) {
                    mav.addObject("username", "用户名不能为空");
                }
                return mav;
            }
            ...
            try {
                ...
                if (null != retryCount){
                    mav.addObject("password", "登录密码错误，剩余 ".concat(Integer.toString(passwordRetry - retryCount.intValue())).concat(" 次"));
                }
            } catch (ExcessiveAttemptsException e) {
                Cache<String, AtomicInteger> passwordRetryCache = cacheManager.getCache("passwordRetryCache");
                AtomicInteger retryCount = passwordRetryCache.get(user.getUsername());
                logger.debug("登录失败次数过多，请稍后重试");
                mav.addObject("password", "登录失败次数过多，请稍后重试");
            } catch (LockedAccountException e) {
                logger.debug("帐号已被锁定. " + token.getPrincipal());
                mav.addObject("password", "帐号已被锁定");
            } catch (DisabledAccountException e) {
                logger.debug("帐号已被禁用. " + token.getPrincipal());
                mav.addObject("password", "帐号已被禁用");
            } catch (ExpiredCredentialsException e) {
                logger.debug("帐号已过期. " + token.getPrincipal());
                mav.addObject("password", "帐号已过期");
            } catch (UnknownAccountException e) {
                logger.debug("帐号不存在. " + token.getPrincipal());
                mav.addObject("username", "帐号不存在");
            } catch (UnauthorizedException e) {
                logger.debug("您没有得到相应的授权！" + e.getMessage());
                mav.addObject("username", "您没有得到相应的授权");
            }
        }
