...
import com.beeant.common.utils.RandomString;
...
public class AuthorizingRealmExt extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authenticationToken) throws AuthenticationException {
        ...
        Message<List<AppUser>> appUser = appUserService.selectByCondition(sysUserExample);
        if (!appUser.isSuccess()) {
            throw new UnknownAccountException("用户不存在");
        }

        AppUser user = appUser.getData().get(0);
        // 检查用户是否禁用
        if (user.getStatus().equalsIgnoreCase(EnumStatus.INACTIVE.value())) {
            throw new DisabledAccountException();
        }
        return new SimpleAuthenticationInfo(user, user.getPassword(), ByteSource.Util.bytes(user.getPassword()), user.getUserName());
    }
